...
class ViewController: UIViewController {
    ...
    override func viewDidAppear(animated: Bool) {
        ...
        self.myModel.stopWatchTimer = NSTimer(timeInterval: 1.0/10.0, target: self, selector: "updateTimer", userInfo: nil, repeats: true)
    ...
    func updateTimer()
    {
        if(self.myModel.timing) {
            ...
            var dateFormmatter = NSDateFormatter()
            dateFormmatter.timeZone = NSTimeZone(abbreviation: "GMT")
            dateFormmatter.setLocalizedDateFormatFromTemplate("HH:mm:ss.SSS")
            var timeString: String = dateFormmatter.stringFromDate(timerDate)
    ...
    @IBAction func startButtonPressed(sender: UIButton) {
        ...
        if(self.myModel.timing) {
            ...
            if(self.displayLabel.text == "00:00:00.000") {
                ...
                // timer has been paused, so we need to offset the startTime
